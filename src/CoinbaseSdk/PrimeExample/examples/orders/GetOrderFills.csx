...
else
{
    PrettyPrinter.PrintUsage(
        "dotnet script GetOrderFills.csx <order-id>", 
        "dotnet script GetOrderFills.csx sample-order-id-12345");
    return;
}

var request = new GetOrderFillsRequest.GetOrderFillsRequestBuilder()
...
try
{
    var response = ordersService.GetOrderFills(request);
    PrettyPrinter.PrintResponse("GetOrderFillsResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving order fills", ex);
