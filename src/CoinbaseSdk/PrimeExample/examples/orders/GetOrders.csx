...
var request = new GetOrdersRequest.GetOrdersRequestBuilder()
...
try
{
    var response = ordersService.GetOrders(request);
    PrettyPrinter.PrintResponse("GetOrdersResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving orders", ex);
}