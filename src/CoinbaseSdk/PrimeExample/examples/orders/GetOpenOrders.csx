...
var request = new GetOpenOrdersRequest.GetOpenOrdersRequestBuilder()
...
try
{
    var response = ordersService.GetOpenOrders(request);
    PrettyPrinter.PrintResponse("GetOpenOrdersResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving open orders", ex);
}