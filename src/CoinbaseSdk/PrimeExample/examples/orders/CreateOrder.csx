...
// Sample order data - replace with actual values
var request = new CreateOrderRequest.CreateOrderRequestBuilder()
    .WithPortfolioId(portfolioId)
    .WithProductId("BTC-USD")
    .WithSide("BUY")
    .WithOrderType("LIMIT")
    .WithBaseQuantity("0.001")
    .WithLimitPrice("50000.00")
    .WithClientOrderId("sample-order-" + DateTime.UtcNow.Ticks)
    .WithTimeInForce("GTC")
    .Build();
...
catch (Exception ex)
{
    ...
    Console.WriteLine("Note: Replace sample order parameters with valid trading values");
}