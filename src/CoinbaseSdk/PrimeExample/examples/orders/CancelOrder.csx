...
else
{
    PrettyPrinter.PrintUsage(
        "dotnet script CancelOrder.csx <order-id>", 
        "dotnet script CancelOrder.csx sample-order-id-12345");
    return;
}

var request = new CancelOrderRequest.CancelOrderRequestBuilder()
...
{
    PrettyPrinter.PrintError("Error canceling order", ex);
    Console.WriteLine("Note: Use GetOpenOrders to find valid order IDs to cancel");
}