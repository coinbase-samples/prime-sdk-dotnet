...
var request = new GetPortfolioBalancesRequest.GetPortfolioBalancesRequestBuilder()
    .WithPortfolioId(portfolioId)
    .Build();

try
{
    var response = balancesService.GetPortfolioBalances(request);
    PrettyPrinter.PrintResponse("GetPortfolioBalancesResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving portfolio balances", ex);
}