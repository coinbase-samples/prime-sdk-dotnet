...
public static class ListAssets
{
    ...
    public static bool Run(string? entityId = null)
    {
        try
        {
            ...
            entityId ??= Environment.GetEnvironmentVariable("COINBASE_PRIME_ENTITY_ID");
            if (string.IsNullOrEmpty(entityId))
            {
                PrettyPrinter.PrintError("Error", new ArgumentException("Entity ID is required. Provide via parameter or COINBASE_PRIME_ENTITY_ID environment variable."));
                return false;
            }
