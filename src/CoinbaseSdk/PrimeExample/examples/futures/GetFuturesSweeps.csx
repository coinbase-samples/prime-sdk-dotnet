...
var request = new GetFuturesSweepsRequest.GetFuturesSweepsRequestBuilder()
    .WithEntityId(entityId)
    .Build();

try
{
    var response = futuresService.GetFuturesSweeps(request);
    PrettyPrinter.PrintResponse("GetFuturesSweepsResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving futures sweeps", ex);
}