...
// Sample futures sweep configuration - replace with actual values
var request = new ScheduleFuturesSweepRequest.ScheduleFuturesSweepRequestBuilder()
    .WithEntityId(entityId)
    .WithAmount("100.00") // Sample amount - replace with actual value
    .WithCurrency("USD") // Sample currency - replace with actual value
    .Build();

try
{
    var response = futuresService.ScheduleFuturesSweep(request);
    PrettyPrinter.PrintResponse("ScheduleFuturesSweepResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error scheduling futures sweep", ex);
    Console.WriteLine("Note: Replace sample amount and currency with valid values");
}