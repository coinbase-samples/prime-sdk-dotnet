...
else
{
    PrettyPrinter.PrintUsage(
        "dotnet script CancelFuturesSweep.csx <sweep-id>", 
        "dotnet script CancelFuturesSweep.csx sample-sweep-id-12345");
    return;
}

var request = new CancelFuturesSweepRequest.CancelFuturesSweepRequestBuilder()
...
try
{
    var response = futuresService.CancelFuturesSweep(request);
    PrettyPrinter.PrintResponse("CancelFuturesSweepResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error canceling futures sweep", ex);
    Console.WriteLine("Note: Use GetFuturesSweeps to find valid sweep IDs");
}