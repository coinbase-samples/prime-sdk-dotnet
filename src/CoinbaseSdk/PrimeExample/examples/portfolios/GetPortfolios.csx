...
var request = new GetPortfoliosRequest.GetPortfoliosRequestBuilder()
    .Build();

try
{
    var response = portfoliosService.GetPortfolios(request);
    PrettyPrinter.PrintResponse("GetPortfoliosResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving portfolios", ex);
}