...
// Sample quote request data - replace with actual values
var request = new CreateQuoteRequestRequest.CreateQuoteRequestRequestBuilder()
    .WithPortfolioId(portfolioId)
    .WithFromAsset("USD")
    .WithToAsset("BTC")
    .WithQuantity("1000.00")
    .WithSide("BUY")
    .Build();

try
{
    var response = portfoliosService.CreateQuoteRequest(request);
    PrettyPrinter.PrintResponse("CreateQuoteRequestResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error creating quote request", ex);
    Console.WriteLine("Note: Replace sample quote parameters with valid values");
}