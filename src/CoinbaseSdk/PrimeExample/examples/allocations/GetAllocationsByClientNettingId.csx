...
else
{
    PrettyPrinter.PrintUsage(
        "dotnet script GetAllocationsByClientNettingId.csx <netting-id>", 
        "dotnet script GetAllocationsByClientNettingId.csx sample-netting-12345");
    return;
}

var request = new GetAllocationsByClientNettingIdRequest.GetAllocationsByClientNettingIdRequestBuilder()
    .WithPortfolioId(portfolioId)
    .WithNettingId(nettingId)
    .Build();
...
{
    PrettyPrinter.PrintError("Error retrieving allocations by netting ID", ex);
}