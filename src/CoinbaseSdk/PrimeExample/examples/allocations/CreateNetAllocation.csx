...
// Sample net allocation data - replace with actual values
var request = new CreateNetAllocationRequest.CreateNetAllocationRequestBuilder()
    .WithClientNettingId("sample-netting-" + DateTime.UtcNow.Ticks)
    .WithSourcePortfolioId("sample-source-portfolio-id")
    .WithTargetPortfolioId("sample-target-portfolio-id")
    .WithSymbol("BTC")
    .WithQuantity("0.01")
    .Build();
...
catch (Exception ex)
{
    ...
    Console.WriteLine("Note: Replace sample portfolio IDs and allocation parameters with valid values");
}