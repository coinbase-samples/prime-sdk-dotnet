...
var rootCommand = new RootCommand("Get Market Data")
{
    entityIdOption,
};
...
rootCommand.SetHandler((entityId) =>
{
    ...
    try
    {
        ...
        var request = new GetMarketDataRequest(entityId);

        PrettyPrinter.PrintResponse("GetMarketDataRequest", request);

        var response = financingService.GetMarketData(request);

        PrettyPrinter.PrintResponse("GetMarketDataResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error calling GetMarketData", ex);
        Environment.ExitCode = 1;
    }
}, entityIdOption);
