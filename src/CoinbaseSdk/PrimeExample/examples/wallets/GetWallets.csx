...
var request = new GetWalletsRequest.GetWalletsRequestBuilder()
    .WithPortfolioId(portfolioId)
    .Build();

try
{
    var response = walletsService.GetWallets(request);
    PrettyPrinter.PrintResponse("GetWalletsResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving wallets", ex);
}