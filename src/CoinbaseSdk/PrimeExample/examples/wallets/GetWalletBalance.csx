...
else
{
    PrettyPrinter.PrintUsage(
        "dotnet script GetWalletBalance.csx <wallet-id>", 
        "dotnet script GetWalletBalance.csx sample-wallet-id-12345");
    return;
}

var request = new GetWalletBalanceRequest.GetWalletBalanceRequestBuilder()
...
try
{
    var response = walletsService.GetWalletBalance(request);
    PrettyPrinter.PrintResponse("GetWalletBalanceResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving wallet balance", ex);
