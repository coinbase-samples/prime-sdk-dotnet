...
else
{
    PrettyPrinter.PrintUsage(
        "dotnet script GetWalletTransactions.csx <wallet-id>", 
        "dotnet script GetWalletTransactions.csx sample-wallet-id-12345");
    return;
}

var request = new GetWalletTransactionsRequest.GetWalletTransactionsRequestBuilder()
...
try
{
    var response = walletsService.GetWalletTransactions(request);
    PrettyPrinter.PrintResponse("GetWalletTransactionsResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving wallet transactions", ex);
    Console.WriteLine("Note: Use GetWallets to find valid wallet IDs");
}