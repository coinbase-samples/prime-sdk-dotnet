...
var request = new GetMarginSummariesRequest.GetMarginSummariesRequestBuilder()
    .WithEntityId(entityId)
    .Build();

try
{
    var response = entitiesService.GetMarginSummaries(request);
    PrettyPrinter.PrintResponse("GetMarginSummariesResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving margin summaries", ex);
}