...
var request = new ListAggregateEntityPositionsRequest.ListAggregateEntityPositionsRequestBuilder()
    .WithEntityId(entityId)
    .Build();

try
{
    var response = entitiesService.ListAggregateEntityPositions(request);
    PrettyPrinter.PrintResponse("ListAggregateEntityPositionsResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving aggregate entity positions", ex);
}