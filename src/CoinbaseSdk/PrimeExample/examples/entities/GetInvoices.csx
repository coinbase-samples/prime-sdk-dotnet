...
var request = new GetInvoicesRequest.GetInvoicesRequestBuilder()
    .WithEntityId(entityId)
    .Build();

try
{
    var response = entitiesService.GetInvoices(request);
    PrettyPrinter.PrintResponse("GetInvoicesResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving invoices", ex);
}