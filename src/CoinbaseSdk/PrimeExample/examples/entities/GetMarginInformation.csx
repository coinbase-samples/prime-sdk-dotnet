...
var request = new GetMarginInformationRequest.GetMarginInformationRequestBuilder()
    .WithEntityId(entityId)
    .Build();

try
{
    var response = entitiesService.GetMarginInformation(request);
    PrettyPrinter.PrintResponse("GetMarginInformationResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving margin information", ex);
}