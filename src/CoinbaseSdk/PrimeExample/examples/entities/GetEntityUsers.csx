...
var request = new GetEntityUsersRequest.GetEntityUsersRequestBuilder()
    .WithEntityId(entityId)
    .Build();

try
{
    var response = entitiesService.GetEntityUsers(request);
    PrettyPrinter.PrintResponse("GetEntityUsersResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving entity users", ex);
}