...
// Optional: Set date range for interest accruals
...
var request = new GetInterestAccrualsRequest.GetInterestAccrualsRequestBuilder()
...
try
{
    var response = entitiesService.GetInterestAccruals(request);
    PrettyPrinter.PrintResponse("GetInterestAccrualsResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving interest accruals", ex);
}