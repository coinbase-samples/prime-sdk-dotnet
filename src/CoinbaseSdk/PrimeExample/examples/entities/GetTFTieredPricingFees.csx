...
var request = new GetTFTieredPricingFeesRequest.GetTFTieredPricingFeesRequestBuilder()
    .WithEntityId(entityId)
    .Build();

try
{
    var response = entitiesService.GetTFTieredPricingFees(request);
    PrettyPrinter.PrintResponse("GetTFTieredPricingFeesResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving TF tiered pricing fees", ex);
}