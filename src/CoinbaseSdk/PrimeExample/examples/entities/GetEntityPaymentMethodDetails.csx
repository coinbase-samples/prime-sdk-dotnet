...
using CoinbaseSdk.Prime.Entities;
...
var entitiesService = new EntitiesService(client);
...
else
{
    PrettyPrinter.PrintUsage(
        "dotnet script GetEntityPaymentMethodDetails.csx <payment-method-id>", 
        "dotnet script GetEntityPaymentMethodDetails.csx sample-payment-method-id");
    return;
}

var request = new GetEntityPaymentMethodDetailsRequest.GetEntityPaymentMethodDetailsRequestBuilder()
...
try
{
    var response = entitiesService.GetEntityPaymentMethodDetails(request);
    PrettyPrinter.PrintResponse("GetEntityPaymentMethodDetailsResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving payment method details", ex);
    Console.WriteLine("Note: Use GetEntityPaymentMethods to find valid payment method IDs");
}