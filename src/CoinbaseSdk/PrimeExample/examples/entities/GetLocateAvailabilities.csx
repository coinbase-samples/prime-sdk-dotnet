...
var request = new GetLocateAvailabilitiesRequest.GetLocateAvailabilitiesRequestBuilder()
    .WithEntityId(entityId)
    .Build();

try
{
    var response = entitiesService.GetLocateAvailabilities(request);
    PrettyPrinter.PrintResponse("GetLocateAvailabilitiesResponse", response);
}
catch (Exception ex)
{
    PrettyPrinter.PrintError("Error retrieving locate availabilities", ex);
}